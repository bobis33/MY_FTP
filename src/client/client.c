...
static void create_new_client(struct client_s *client, const int new_fd) {
    for (int index = 0; index < MAX_CLIENTS; index++) {
        if (client->clients[index].username == NULL) {
            ...
            new_fd > client->max_fd ? client->max_fd = new_fd : 0;
            break;
        }
    }
}
...
static int accept_new_client(struct client_s *client, const int server_fd) {
    int new_fd = accept(server_fd, (struct sockaddr *)&client->client_addr, &client->client_len);
    ...
    if (client->clients[MAX_CLIENTS - 1].username != NULL) {
        close(new_fd);
        return ERROR; // Too many clients
    }
    create_new_client(client, new_fd);
    return new_fd;
}

static void process_ready_fds(struct client_s *client, const int serv_fd, const int fd) {
...
int handle_clients(struct server_s *server) {
    ...
    memset(client->clients, 0, sizeof(client->clients)); // Initialize client structures
    for (int i = 0; i < MAX_CLIENTS; i++) {
